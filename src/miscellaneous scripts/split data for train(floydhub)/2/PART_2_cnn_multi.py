...
def cnn_model(X_train, y_train, nb_filters, weights, channels, nb_gpus, nb_classes, kernel_size, batch_size, nb_epoch):
    ...
    model = Sequential()
    model.add(Conv2D(nb_filters, (kernel_size[0], kernel_size[1]), padding = 'valid', strides = 4,
    ...
    kernel_size = (16, 16)
    model.add(Conv2D(64, (kernel_size[0], kernel_size[1]), activation = 'relu'))
    model.add(MaxPooling2D(pool_size = (2, 2)))
    ...
    model.load_weights("/floyd/input/output1/saved_model/PART_1_cnn_multi_recall_0.4756.h5")
...
if __name__ == '__main__':
    ...
    X = np.load("/floyd/input/data/2_uncomp_save.npy")
    y = np.array(labels['level'][26600:53200])
    ...
    save_model(model = model, score = recall, model_name = "PART_2_cnn_multi")
    print("Completed")
