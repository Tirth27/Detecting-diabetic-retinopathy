...
def plot_classification_frequency(df, category, file_name, convert_labels = False):
    ...
    if convert_labels == True:
        label['level'] = change_labels(label, 'level')
     
    sns.set(style="whitegrid", color_codes=True)
    sns.countplot(x=category, data=label)
...
if __name__ == '__main__':
    label = pd.read_csv("/home/tirth/Diabetic Retinopathy/label visualize/trainlabel_master_v2.csv")   
    plot_classification_frequency(label, "level", "Retinopathy_vs_Frequency_All")
